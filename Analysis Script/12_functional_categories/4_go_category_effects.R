...
message("GO external category gene-effect summary started: ", Sys.time())
...
gene_set_file <- file.path(tables_dir, "go_external_gene_sets_20260619.csv")
...
for (i in seq_len(nrow(blocks))) {
  ...
  for (category in categories) {
    genes <- unique(gene_sets$gene[gene_sets$species == block$species & gene_sets$category == category])
    sub <- block_effects[block_effects$assay_symbol %in% genes, ]
    vals <- sub$aged_minus_young
    rows[[k]] <- data.frame(
      analysis_layer = "go_external_gene_effect_table",
...
summary_df$signed_rank_fdr_within_go_probe <- p.adjust(summary_df$signed_rank_p_against_zero, method = "BH")
write.csv(summary_df, file.path(tables_dir, "go_external_category_gene_effect_summary_20260619.csv"), row.names = FALSE)
...
write.csv(dataset_level, file.path(tables_dir, "go_external_category_dataset_level_20260619.csv"), row.names = FALSE)
write.csv(dataset_blocked, file.path(tables_dir, "go_external_category_dataset_blocked_20260619.csv"), row.names = FALSE)
...
plot_order <- c(
  ...
  "go_metabolic_support_union_prespecified",
  "go_metabolic_support_union_no_hmgcs2",
...
ggsave(file.path(figures_dir, "go_external_category_gene_effect_heatmap_20260619.pdf"), p, width = 183, height = 125, units = "mm")
ggsave(file.path(figures_dir, "go_external_category_gene_effect_heatmap_20260619.png"), p, width = 183, height = 125, units = "mm", dpi = 450)

sink(file.path(logs_dir, "go_external_gene_effect_summary_session_info_20260619.txt"))
cat("GO external category gene-effect summary completed: ", as.character(Sys.time()), "\n\n", sep = "")
print(sessionInfo())
sink()

message("GO external category gene-effect summary completed: ", Sys.time())
