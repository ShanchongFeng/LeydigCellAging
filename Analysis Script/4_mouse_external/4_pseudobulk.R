...
OUT <- path.expand(Sys.getenv(
  "GSE303193_PSEUDOBULK_ROOT",
  unset = file.path(Sys.getenv("HOME"), "GSE303193_PSEUDOBULK_ROOT")
))
DIRS <- list(
...
on.exit(writeLines(capture.output(sessionInfo()), file.path(DIRS$session, "GSE303193_sessionInfo.txt")), add = TRUE)

obj_file <- Sys.glob(file.path(LOCKED_ROOT, "rds/primary_locked/GSE303193/*3_filtered_clustered_seurat.rds"))[1]
if (is.na(obj_file) || !file.exists(obj_file)) stop("Missing RDS input for GSE303193: ", obj_file)
...
boundaries <- list(
  cluster16 = c('16'),
  cluster16_17 = c('16', '17'),
  cluster16_17_19 = c('16', '17', '19')
)
target_genes <- c('Hmgcs2', 'Cyp11a1', 'Cyp17a1', 'Star', 'Hsd3b1', 'Lhcgr', 'Nr5a1', 'Foxo3', 'Insl3', 'Hdac1')

write.csv(data.frame(role="input_rds", path=obj_file, exists=file.exists(obj_file)),
          file.path(DIRS$metadata, "GSE303193_input_paths.csv"), row.names=FALSE)

sample_meta <- obj@meta.data |>
  as.data.frame() |>
  count(.data[["sample_id"]], .data[["group"]], name = "n_cells_total")
colnames(sample_meta)[1:2] <- c("sample_id", "group")
write.csv(sample_meta, file.path(DIRS$metadata, "GSE303193_sample_metadata.csv"), row.names=FALSE)
...
run_boundary <- function(boundary_name, clusters_requested) {
  ...
  colnames(info) <- c("sample_id", "group")
  ...
  design <- data.frame(
    ...
    n_cells_in_boundary = as.integer(table(factor(ids, levels = info[[ "sample_id" ]]))),
    stringsAsFactors = FALSE
  )

  write.csv(pb_mat, file.path(DIRS$tables, paste0("GSE303193_", boundary_name, "_pseudobulk_counts.csv")), quote=FALSE)
  write.csv(design, file.path(DIRS$metadata, paste0("GSE303193_", boundary_name, "_design_matrix.csv")), row.names=FALSE)
  ...
  keep_genes <- rowSums(pb_mat >= 10) >= max(2, min(table(info$group)))
  sparsity$in_DESeq2_filter <- sparsity$gene %in% rownames(pb_mat)[keep_genes]
  write.csv(sparsity, file.path(DIRS$tables, paste0("GSE303193_", boundary_name, "_target_sparsity.csv")), row.names=FALSE)
  ...
  if (sum(keep_genes) >= 50 && nlevels(droplevels(info$group)) == 2) {
    ...
    write.csv(res, file.path(DIRS$tables, paste0("GSE303193_", boundary_name, "_DESeq2_Aged_vs_Young.csv")), row.names=FALSE)
  ...
  write.csv(target_res, file.path(DIRS$tables, paste0("GSE303193_", boundary_name, "_target_genes_DESeq2.csv")), row.names=FALSE)
  target_res
}

all_targets <- bind_rows(lapply(names(boundaries), function(nm) run_boundary(nm, boundaries[[nm]])))
write.csv(all_targets, file.path(DIRS$tables, "GSE303193_all_boundaries_target_genes_DESeq2.csv"), row.names=FALSE)
